...
public class ConcurrentExecutor {
    ...
    public ConcurrentExecutor(ExecutionStatistics statistics, ExecutionContext context) {
        this.statistics = statistics;
        this.context = context;
    }

    public void runConcurrent(final Runnable unit, int numberOfConcurrentExecutions) {
	...
	public void runConcurrent(final Runnable... units) {
		...
		for (Future<Long> result : returnValues)
			try {
                statistics.store(context, result.get());
			} catch (Exception e) {
				//should never happen
